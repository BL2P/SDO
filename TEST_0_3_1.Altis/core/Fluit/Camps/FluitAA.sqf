...
aa_camp1 = {
	...
	if (random 1 < 0.5) then {
		...
		_gun1 setDir _dir;
		_gunner1 = _campgun1group createUnit [SDO_u_AO_rifle, _pos, [], 0, "NONE"];
		waitUntil {alive _gunner1};
		_gunner1 assignAsGunner _gun1;
		_gunner1 moveInGunner _gun1;
	};
	
	_soldier = _campgroup createUnit [SDO_u_AO_sl, _housepos, [], 0, "NONE"];
	doStop _soldier;
	for "_c" from 1 to (1 + round (random 1)) do { 
		_newpos = [_pos, ceil (random 10), random 360] call BIS_fnc_relPos;
		_soldier = _campgroup createUnit [SDO_u_AO_aa, _newpos, [], 0, "NONE"];
		doStop _soldier;
		_newpos = [_pos, ceil (random 10), random 360] call BIS_fnc_relPos;
		_soldier = _campgroup createUnit [SDO_u_AO_ammo, _newpos, [], 0, "NONE"];
		doStop _soldier;
	};
	true;
};
...
aa_camp2 = {
	...
	if (random 1 < 0.75) then {
		...
		_gunner1 = _campgun1group createUnit [SDO_u_AO_ammo, _newpos, [], 0, "NONE"];
		waitUntil {alive _gunner1};
		_gunner1 assignAsGunner _gun1;
		_gunner1 moveInGunner _gun1;
	};
	...
	sleep 0.5;
	_soldier = _campgroup createUnit [SDO_u_AO_sl, getPos _tower, [], 0, "NONE"];
	doStop _soldier;
	for "_c" from 1 to (1 + round (random 1)) do
	{ 
