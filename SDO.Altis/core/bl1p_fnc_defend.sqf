			...
			*/
			...
			while {_defend} do
			{
				...
				for "_x" from 1 to _AttackAmount do 
				{
					...
					_randomPos = [getMarkerPos currentAO, _distance] call SDO_fnc_randomPosbl1p;
					...
					if ((count _randomPos) == 3) then 
					{
						...
						if (random 10 < 9) then 
						{
							...
							{
								_x createUnit [_randomPos, _inf_Patrol];
							...
							/// Create marker for debug
								if(DEBUG) then
									{
										_name = format ["%1%2",name (leader _inf_Patrol),_x];
										createMarker [_name,getPos (leader _inf_Patrol)];
										_name setMarkerType "o_unknown";
										_name setMarkerText format ["Defence Inf %1",_x];
										_name setMarkerColor "ColorRed";
										[_inf_Patrol,_name] spawn
										{
						...
						else
						{
							...
							_inf_Patrol = createGroup east;
							_veh = [_roadpos,0,(SDO_Armed_Cars call Bis_fnc_SelectRandom),_inf_Patrol] call BIS_fnc_spawnVehicle;
								...
								if(DEBUG) then
									{
										...
										createMarker [_name,getPos (leader _inf_Patrol)];
										_name setMarkerType "o_unknown";
										_name setMarkerText format ["Defence Moto  %1",_x];
										_name setMarkerColor "ColorRed";
										[_inf_Patrol,_name] spawn
										{
											private["_group","_marker"];
						...
						//[_inf_Patrol, getMarkerPos currentAO,250] call SDO_fnc_spawn2_perimeterPatrolBL1P;
						//waituntil {sleep 1;alive (leader _inf_Patrol)};
						sleep 0.5;
						[_inf_Patrol, getMarkerPos currentAO,100] call SDO_fnc_spawn2_randomPatrolBL1P;
			...
			if (AttackWon) then 
			{
				...
				[
					...
					"<t align='center' size='2.2'>Defended Target</t><br/><t size='1.5' align='center' color='#FFCF11'>%1</t><br/>____________________<br/>Congratulations you held out!<br/>____________________<br/>Assets are Available",currentAO
				];
				if (ELAPSED) then 
					{
						_AttackResultHint = format
						[
							...
							"<t align='center' size='2.2'>Defended Target</t><br/><t size='1.5' align='center' color='#FFCF11'>%1</t><br/>____________________<br/>Enemy Forces have retreated.<br/>____________________<br/>Assets are Available",currentAO
						];
					};
