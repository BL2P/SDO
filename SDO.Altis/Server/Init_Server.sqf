...
//--- Move base stuff
	...
	execvm "Server\Base\MoveAndInitialise.sqf";
...
//--- bl1p read the defend function
	execVm "Server\Server_Scripts\bl1p_fnc_defend.sqf";

//--- server add tickets
	execVM "Server\Server_Scripts\TicketCounter.sqf";
...
//--- Clean script
		_null = [] execVM "Server\Server_Scripts\clearItems.sqf";
...
//--- Run AO and/or DEP victory scripts
	...
	if (PARAMS_AOTOGGLE == 1) then 
		{ 
			_AO_UnitTypesM = execVM "Server\Server_Scripts\AO_UnitTypes_Main.sqf";
			waitUntil{scriptDone _AO_UnitTypesM}; 
			_PointlessVariable1 = execVM "Server\Server_Scripts\AOLoop.sqf";
		};
		
	if (PARAMS_DEP_VICTORY == 1) then 
		{ 
			_PointlessVariable2 = execVM "Server\Server_Scripts\DEPVictory.sqf";
		};
		
	//--- start convoys
	if (PARAMS_ConvoyChance > 0) then 
	{ 
	//--- Not working Dont use yet use
	//_null = [] execVM "Server\Reinforcement\AOConvoy.sqf"; 
	};
