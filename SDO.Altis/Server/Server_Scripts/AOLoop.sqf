...
//--- bl1p moved ao unit creation into seperate script
	_handle2 = execVM "Server\Server_Scripts\Create_Units.sqf";
	waitUntil{scriptDone _handle2};

//--- create some random aa opsitions
	_null = [] execVM "Server\Server_Scripts\AAPosCreation.sqf";
...
/* =============================================== */
	{
		...
		//--- bl1p create reinforments on Server
			...
			if (PARAMS_HeavyReinforcement > 0) then { _null = [] execVM "Server\Reinforcement\AOHeavyReinforce.sqf"; };
			if (PARAMS_AOReinforcement  > 0) then { _null = [] execVM "Server\Reinforcement\AOReinforcement.sqf"; };
		...
		if (count _targets > PARAMS_AOENDCOUNT) then 
			{
				...
				//--- clean up arrays by fluit and bl1p
					...
					_handleAOclean = [_arraystocleanup] execVM "Server\Server_Scripts\AOCleanup.sqf";
